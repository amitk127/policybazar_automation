...
class PolicyDatabase:
    ...
    def get_ids(connection) -> List[int]:
        ...
        try:
            ...
            logger.info("Error fetching IDs",exc_info=True)
            return ids
    ...
    def update_ui_error_remarks(connection, record_id):
        ...
        try:
            ...
            logger.info(f"Error updating UI Error remarks for ID {record_id}: {e}")
            connection.rollback()
    ...
    def update_ncb_elements_missing_remark(connection, record_id):
        ...
        try:
            ...
            end_time = datetime.datetime.now().replace(microsecond=0)
            cursor.execute(sql_query,
                           ('NCB option is missing', 'NCB option is missing', 'NCB option is missing', end_time, record_id))
            connection.commit()
            cursor.close()
            logger.info(f"Updated NCB elements missing remarks for ID: {record_id}")

        except Exception as e:
            logger.info(f"Error updating NCB elements missing remarks for ID {record_id}: {e}")
            connection.rollback()
    ...
    def update_new_ui_popup_remarks(connection, record_id):
        ...
        try:
            ...
            logger.info(f"Error updating popup failure for ID {record_id}: {e}")
            connection.rollback()
    ...
    def update_ncb_not_found_remarks(connection, record_id):
        ...
        try:
            ...
            logger.info(f"Error updating NCB not found remarks for ID {record_id}: {e}")
            connection.rollback()
    @staticmethod
    def update_case_start_time(connection, record_id, start_time):
        ...
        try:
            ...
            logger.info(f"Error updating start time for ID {record_id}: {e}")
            connection.rollback()
    ...
    def update_case_end_time(connection, record_id, end_time):
        ...
        try:
            ...
            logger.info(f"Error updating end time for ID {record_id}: {e}")
            connection.rollback()
    ...
    def update_duration(connection, record_id, duration_str):
        ...
        try:
            ...
            logger.info(f"Error updating duration for ID {record_id}: {e}")
            connection.rollback()
    ...
    def _get_insurer_params(details: Optional[InsuranceDetails]) -> List[Optional[str]]:
        ...
        return [
            ...
            details.carvalue,
            details.damage,
            details.third_party_cover_premium,
            details.zero_depreciation,
            details.roadside,
            details.engine_protection_cover,
            details.consumables,
            details.package_premium,
            details.premium,
            details.premium_other_addon_autoselected_bundle,
            details.tyre,
            details.gap_cover
        ]